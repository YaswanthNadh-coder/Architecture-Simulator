// src/components/layout/Sidebar.tsx — full replacement
import { Link, useLocation, useNavigate } from 'react-router-dom';
import {
  Box, FileCode2, CreditCard, Settings, LogOut,
...
export const Sidebar = () => {
  ...
  const navItems = [
    ...
    { icon: <GraduationCap size={22} />, path: '/grading',   label: 'Grading'    },
  ];
  ...
  return (
    <div className="w-16 h-full bg-bg-panel border-r border-border-subtle flex flex-col items-center py-6 shadow-xl z-20 relative shrink-0">
      ...
      {/* Avatar — shows initials if signed in, generic icon if not */}
      <div className="mb-8">
      ...
      <div className="flex flex-col gap-6 w-full items-center mt-auto">
        ...
        {/* Logout — only shown if the user is signed in */}
        {profile && (
          <button
        ...
        {/* Sign in — only shown if not signed in */}
        {!profile && (
          <Link
            to="/login"
            className="relative w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-200 group text-text-muted hover:text-white hover:bg-white/5"
          >
            <div className="group-hover:scale-110 transition-transform">
              <User size={22} />
            </div>
